...
export async function assertSafeAndPin(
  rawUrl: string,
  lookupFn: DnsLookup = dnsLookup,
): Promise<PinnedTarget> {
  let url: URL;
  try {
    url = new URL(rawUrl);
  } catch {
    throw new SsrfViolation('MALFORMED_URL');
  }

  if (url.protocol !== 'https:')
    throw new SsrfViolation('PROTOCOL', { protocol: url.protocol });
  if (url.username || url.password)
    throw new SsrfViolation('CREDENTIALS_IN_URL');
  if (
    url.port &&
    !ALLOWED_PORTS.has(url.port) &&
    !(process.env.NODE_ENV === 'test' && url.port === '8443')
  )
    throw new SsrfViolation('NONSTANDARD_PORT');

  const host = url.hostname.toLowerCase();
  if (!ALLOWED_HOSTS.some((rx) => rx.test(host)))
    throw new SsrfViolation('HOST_NOT_ALLOWLISTED', { host });
  ...
  for (const { address } of addrs) {
    if (!isPublicUnicast(address))
      throw new SsrfViolation('PRIVATE_ADDRESS', { address });
  }
