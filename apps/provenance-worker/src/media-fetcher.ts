...
export async function fetchPinned(
  ...
  for (let hop = 0; hop <= MAX_REDIRECTS; hop++) {
    ...
    const agentOptions = {
      ...
      (global as any).TEST_CA_CERT
        ? { ca: (global as any).TEST_CA_CERT }
        : {}),
    };
    ...
    const res = await new Promise<http.IncomingMessage>((resolve, reject) => {
      const req = requestFn(url, {
        ...
        method: 'GET',
        headers: {
          'user-agent': 'AntiAI-ProvenanceBot/1.0 (+https://antiai.me/bot)',
        },
    ...
    if (declared > MAX_BYTES)
      throw new SsrfViolation('TOO_LARGE', { declared });
