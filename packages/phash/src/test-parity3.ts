...
export async function computePhashSharp(buffer: Buffer): Promise<string> {
  const { data, info } = await sharp(buffer)
    .resize(16, 16, { fit: 'fill', kernel: sharp.kernel.nearest }) // Nearest neighbor
    .raw()
    .ensureAlpha()
...
export async function computePhashCanvas(buffer: Buffer): Promise<string> {
  ...
  // Disable anti-aliasing / smoothing to use nearest-neighbor
  ...
  const hash = blockhash.bmvbhash(
    { width: 16, height: 16, data: imgData.data },
    8
  );
  return hash;
}
